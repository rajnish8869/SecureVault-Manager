...
interface IntruderLogsProps {
    ...
    onViewImage: (img: any) => void;
    onBack: () => void;
}

const EvidenceThumbnail: React.FC<{ item: VaultItem; onClick: () => void }> = ({ item, onClick }) => {
    ...
    return (
        ...
        >
            {src ? (
                <>
                  <img src={src} alt="Evidence" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-100" />
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-vault-600 gap-2">
                    <div className="animate-pulse"><Icons.Camera /></div>
        ...
        <div className="animate-in fade-in slide-in-from-right-8 duration-300 min-h-screen bg-vault-900 flex flex-col font-sans">
          ...
          <header className="sticky top-0 z-40 bg-vault-900/95 backdrop-blur-xl border-b border-vault-800 px-4 py-3 flex items-center justify-between shadow-lg shadow-black/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-lg text-white leading-none">Security Log</h2>
                <p className="text-[10px] text-vault-500 font-mono mt-1 tracking-wider">INTRUSION EVIDENCE</p>
              </div>
            </div>
            <div className={`px-3 py-1 rounded-full border text-xs font-bold font-mono ${logs.length > 0 ? 'bg-red-500/10 border-red-500/20 text-red-400 animate-pulse' : 'bg-vault-800 border-vault-700 text-vault-500'}`}>
                {logs.length} EVENTS
            </div>
          </header>
             ...
             {logs.length === 0 ? (
                 <div className="flex flex-col items-center justify-center h-[60vh] text-vault-600 space-y-4">
                     ...
                     <div className="text-center">
                        ...
                        <p className="text-sm text-vault-500 mt-1">No unauthorized access attempts detected.</p>
                     </div>
                 </div>
             ) : (
                 logs.map((session, index) => (
                     <div key={session.id} className="relative pl-6 pb-6 last:pb-0 before:absolute before:left-[11px] before:top-8 before:bottom-0 before:w-px before:bg-vault-800 last:before:hidden">
                        {/* Timeline Dot */}
                            ...
                            <div className="px-4 py-3 border-b border-vault-700/50 flex justify-between items-center bg-vault-800/50">
                                 <div>
                                     ...
                                     <p className="text-xs text-vault-400 font-mono mt-0.5">
                                         {new Date(session.timestamp).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })} • {new Date(session.timestamp).toLocaleTimeString()}
                                     </p>
                                 </div>
                                 <button 
