...
import { Card, Button, PasswordInput, Toggle, SegmentedControl, Icons } from '../components/UI';
...
interface SettingsViewProps {
  ...
  hasDecoy: boolean;
  onBack: () => void;
  onUpdateCredentials: (oldPw: string, newPw: string, newType: LockType) => Promise<void>;
  onReset: () => void;
  onToggleBio: () => void;
...
export const SettingsView: React.FC<SettingsViewProps> = ({ 
    lockType, bioAvailable, bioEnabled, hasDecoy, onBack, onUpdateCredentials, onReset, onToggleBio, onSetupDecoy, onRemoveDecoy, onOpenIntruder, isProcessing
}) => {
  // Change Credentials State
  const [form, setForm] = useState({ old: '', new: '', confirm: '', type: lockType });
  ...
  const validateDecoy = () => {
      ...
      if (lockType === 'PIN') {
          ...
          if (decoyForm.pass.length < MIN_PASS_LENGTH) {
              setDecoyError(`Decoy Password must be at least ${MIN_PASS_LENGTH} characters`);
              return false;
          }
      }
    ...
    <div className="animate-in fade-in slide-in-from-right-8 duration-300 bg-vault-900 min-h-screen pb-10">
        ...
        <main className="p-4 max-w-2xl mx-auto space-y-8">
            ...
            <section className="space-y-3">
                <h3 className="text-xs font-bold text-vault-500 uppercase tracking-wider px-1">Security Monitoring</h3>
                <Card className="p-1">
                   <button 
                   ...
                   >
                       <div className="flex items-center gap-4">
                           ...
                           <div className="text-left">
                               <h3 className="font-bold text-white group-hover:text-red-400 transition-colors">Intruder Selfie</h3>
                               <p className="text-xs text-vault-400">Capture photos of failed unlock attempts</p>
                           </div>
                       </div>
                       <div className="text-vault-500 group-hover:text-white transition-colors rotate-180"><Icons.ArrowLeft /></div> 
                   </button>
                </Card>
            </section>
            ...
            {/* General Settings */}
            <section className="space-y-3">
                <h3 className="text-xs font-bold text-vault-500 uppercase tracking-wider px-1">General</h3>
                <Card className="divide-y divide-vault-700/50">
                    {/* Biometric */}
                    {bioAvailable && (
                        <div className="p-4 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <div className="text-vault-400"><Icons.Fingerprint /></div>
                                <div>
                                    <h4 className="font-medium text-white">Biometric Unlock</h4>
                                    <p className="text-xs text-vault-400">Use FaceID or Fingerprint</p>
                                </div>
                            </div>
                            <Toggle checked={bioEnabled} onChange={onToggleBio} />
                        </div>
                    )}
                    ...
                    <div className="p-4 space-y-4">
                        <div className="flex items-center justify-between">
                             <div className="flex items-center gap-3">
                                 <div className="text-amber-500"><Icons.Shield /></div>
                                 <div>
                                     <h4 className="font-medium text-white">Coercion Defense</h4>
                                     <p className="text-xs text-vault-400">Setup a decoy vault with fake files</p>
                                 </div>
                             </div>
                             <Toggle 
                                checked={hasDecoy} 
                                onChange={() => hasDecoy ? onRemoveDecoy() : setShowDecoySetup(true)} 
                             />
                        </div>
                            ...
                            <div className="bg-vault-900/50 rounded-xl border border-vault-700 p-4 space-y-4 animate-in slide-in-from-top-2">
                                ...
                                <div className="space-y-3">
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-vault-400">DECOY {lockType}</label>
                                        <PasswordInput 
                                            value={decoyForm.pass} 
                                            onChange={v => setDecoyForm(s => ({...s, pass: v}))} 
                                            placeholder={`Enter Decoy ${lockType === 'PIN' ? `(${PIN_LENGTH} digits)` : ''}`}
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-vault-400">CONFIRM DECOY</label>
                                        <PasswordInput 
                                            value={decoyForm.confirm} 
                                            onChange={v => setDecoyForm(s => ({...s, confirm: v}))} 
                                    ...
                                    {decoyError && <p className="text-xs text-red-400 font-medium">{decoyError}</p>}
                                </div>

                                <div className="flex gap-3 pt-2">
                                    <Button variant="ghost" className="flex-1" onClick={() => { setShowDecoySetup(false); setDecoyForm({pass:'',confirm:''}); }}>Cancel</Button>
                                    <Button className="flex-1 bg-amber-600 hover:bg-amber-700 text-white" onClick={handleDecoySubmit}>Enable Decoy</Button>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            {/* Change Credentials */}
            <section className="space-y-3">
                <h3 className="text-xs font-bold text-vault-500 uppercase tracking-wider px-1">Authentication</h3>
                <Card className="p-6 space-y-6">
                    <div className="space-y-4">
                         <div className="bg-vault-900/50 p-1 rounded-lg border border-vault-700">
                             <SegmentedControl 
                                options={[{label: 'PIN Code', value: 'PIN'}, {label: 'Password', value: 'PASSWORD'}]}
                                value={form.type}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <PasswordInput 
                                    ...
                                    placeholder={form.type === 'PIN' ? `Enter ${PIN_LENGTH} digits` : `Min ${MIN_PASS_LENGTH} characters`}
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-vault-400">CONFIRM NEW</label>
                                <PasswordInput 
                                    value={form.confirm} 
                            ...
                            {credError && <p className="text-xs text-red-400 font-medium bg-red-500/10 p-2 rounded">{credError}</p>}
                        </div>
            ...
            {/* Danger Zone */}
            <section className="space-y-3">
                <h3 className="text-xs font-bold text-red-500/70 uppercase tracking-wider px-1">Danger Zone</h3>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-red-200/60 leading-relaxed">
                                This will permanently delete all encrypted files, intruder logs, and settings. 
